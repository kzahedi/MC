...
%     s1 = files(file_index).s(1:end-1,:);
    ...
    tic
    files(file_index).mcc  = MC_C_dynamic_detail( ...
        files(file_index).w(2:end,:),   ... % w2
        files(file_index).w(1:end-1,:), ... % w1
        files(file_index).s(1:end-1,:), ... % s1
        files(file_index).a(1:end-1,:)  ... % a1
    );
    toc
end
...
for file_index = [1,2,3]
    r = [r, files(file_index).mcc];
end

csvwrite(sprintf('/Users/zahedi/projects/QuantMorphComp/data/mcc_data_%d.csv',bins), r);
